...
const wilayahKerjaData = [
   { id: 'wk1', name: 'WK Rokan', color: '#f59e0b', coords: [[1.5, 100.5], [2.0, 101.5], [1.0, 102.0], [0.5, 101.0]] },
   { id: 'wk2', name: 'WK Mahakam', color: '#10b981', coords: [[-0.5, 117.0], [-1.0, 117.5], [-1.5, 117.0], [-1.0, 116.5]] },
   { id: 'wk3', name: 'WK Natuna', color: '#3b82f6', coords: [[4.0, 107.5], [5.0, 108.5], [4.5, 109.0], [3.5, 108.0]] },
];
...
const AssetMap: React.FC<AssetMapProps> = ({ 
  ...
  const hasWeatherAlert = notifications.some(n => n.type === 'warning' && n.title.includes('WEATHER'));
  ...
  useEffect(() => {
    ...
    if (spatialFilter && !singleAsset) {
       result = result.filter(a => {
          const dist = getDistance(spatialFilter.lat, spatialFilter.lng, a.coordinates.lat, a.coordinates.lng);
  ...
  useEffect(() => {
    ...
    map.on('move', () => {
       ...
       setHoveredAsset(prev => {
         if (prev) {
            try {
              const point = mapInstanceRef.current?.latLngToContainerPoint([prev.coordinates.lat, prev.coordinates.lng]);
              if (point) setTooltipPos(point);
            } catch (e) { /* ignore projection errors during drag */ }
         }
         return prev;
       });
    });
    ...
    return () => {
      ...
      if (mapInstanceRef.current) {
        ...
        logisticsLinesRef.current.forEach(l => l.remove());
  ...
  // Logistics Layer
  useEffect(() => {
     if (!isMapReady || !mapInstanceRef.current || !shorebaseLayerRef.current) return;
     ...
     if (showLogistics && shorebases.length > 0) {
        shorebases.forEach(sb => {
           ...
           `;
           const icon = L.divIcon({ className: 'custom-marker', html: iconHtml, iconSize: [32, 32], iconAnchor: [16, 32] });
           const marker = L.marker([sb.coordinates.lat, sb.coordinates.lng], { icon });
           marker.bindPopup(`<b>${sb.name}</b><br/>${sb.location}`, { closeButton: false });
        ...
        if (selectedAssetId) {
           ...
           if (asset) {
              ...
              shorebases.forEach(sb => {
                 const d = getDistance(asset.coordinates.lat, asset.coordinates.lng, sb.coordinates.lat, sb.coordinates.lng);
                 if (d < minDist) { minDist = d; nearest = sb; }
              });

              if (nearest) {
                 const line = L.polyline(
                    [[asset.coordinates.lat, asset.coordinates.lng], [nearest.coordinates.lat, nearest.coordinates.lng]], 
  ...
  // Weather & Heatmap
  useEffect(() => {
     if (!isMapReady || !mapInstanceRef.current || !weatherLayerRef.current) return;
     
     weatherLayerRef.current.clearLayers();
     if (hasWeatherAlert) {
        L.circle([4.8, 108.0], {
           radius: 150000, color: '#ef4444', fillColor: '#ef4444', fillOpacity: 0.2, weight: 1, dashArray: '10, 5'
        }).addTo(weatherLayerRef.current);
     } else if (showHeatmap) {
        filteredAssets.forEach(asset => {
           L.circle([asset.coordinates.lat, asset.coordinates.lng], {
             radius: 60000, fillColor: '#0ea5e9', fillOpacity: 0.05, color: 'transparent', interactive: false
           }).addTo(weatherLayerRef.current!);
         });
     }
       ...
       if (showZones) {
          zones.forEach(zone => {
            L.circle([zone.coordinates.lat, zone.coordinates.lng], {
              color: zone.color, fillColor: zone.color, fillOpacity: 0.05, radius: zone.radius, weight: 1, dashArray: '4, 4'
       ...
       if (showWK) {
          wilayahKerjaData.forEach(wk => {
             L.polygon(wk.coords as any, {
                color: wk.color, weight: 1, fillColor: wk.color, fillOpacity: 0.1, dashArray: '4, 4'
  ...
  useEffect(() => {
     ...
     if (spatialFilter) {
        L.circle([spatialFilter.lat, spatialFilter.lng], {
           radius: spatialFilter.radius * 1000, color: '#6366f1', weight: 1.5, fillColor: '#6366f1', fillOpacity: 0.05, dashArray: '4, 4'
  ...
  useEffect(() => {
    ...
    if (singleAsset) {
      map.setView([singleAsset.coordinates.lat, singleAsset.coordinates.lng], zoomLevel === 'local' ? 12 : 6);
    } 
    ...
    filteredAssets.forEach(asset => {
      ...
      const getIconColor = (cat: string) => {
        if (cat === 'Onshore Rig') return 'text-indigo-600 bg-indigo-50 border-indigo-200';
        if (cat === 'Offshore Rig') return 'text-sky-600 bg-sky-50 border-sky-200';
        return 'text-orange-600 bg-orange-50 border-orange-200';
      };
      
      const getIconSvg = (cat: string) => {
        if (cat === 'Kapal') return '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="transform: rotate(45deg)"><polygon points="3 11 22 2 13 21 11 13 3 11"></polygon></svg>';
        if (cat === 'Offshore Rig') return '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>';
      ...
      const iconHtml = `
        <div class="relative group transition-transform duration-300 ${isSelected ? 'scale-125 z-50' : 'hover:scale-110 z-10'}">
          <div class="w-8 h-8 rounded-full ${styleClass} flex items-center justify-center border shadow-sm">
            ${iconSvg}
          </div>
          ${asset.status === 'Active' ? '<span class="absolute -top-0.5 -right-0.5 flex h-2.5 w-2.5"><span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span class="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500 border border-white"></span></span>' : ''}
        </div>
      `;

      const customIcon = L.divIcon({ className: 'custom-marker', html: iconHtml, iconSize: [32, 32], iconAnchor: [16, 16] });

      if (!marker) {
        marker = L.marker([asset.coordinates.lat, asset.coordinates.lng], { icon: customIcon }).addTo(map);
      ...
      if (!singleAsset && asset.history.length > 1) {
         if (!polyline) {
            polyline = L.polyline(asset.history, {
               color: asset.category === 'Onshore Rig' ? '#6366f1' : asset.category === 'Offshore Rig' ? '#0ea5e9' : '#f97316',
               weight: 1.5, opacity: 0.4, dashArray: '3, 3'
            }).addTo(map);
  ...
  }, [isMapReady, filteredAssets, singleAsset, zoomLevel, zones, selectedAssetId, onAssetClick, theme]);

  const fitBounds = () => {
     if (mapInstanceRef.current && filteredAssets.length > 0) {
        const group = new L.FeatureGroup(filteredAssets.map(a => L.marker([a.coordinates.lat, a.coordinates.lng])));
        mapInstanceRef.current.fitBounds(group.getBounds().pad(0.1));
     }
  };

  return (
    <div className={`relative w-full ${height} bg-slate-100 dark:bg-slate-900 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm group`}>
      <div ref={mapContainerRef} className="absolute inset-0 z-0 bg-slate-100 dark:bg-slate-950" />
      ...
      {hasWeatherAlert && !singleAsset && (
         <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[400] bg-rose-600 text-white px-3 py-1.5 rounded-lg shadow-sm flex items-center gap-2 animate-pulse">
            ...
            <span className="font-bold text-xs uppercase tracking-wide">Protocol: Red-Natuna</span>
         </div>
      )}
      ...
      <div className="absolute right-4 top-4 z-[400] flex flex-col gap-2">
         {!singleAsset && (
             <button onClick={fitBounds} className="bg-white dark:bg-slate-800 p-2 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 text-slate-500 hover:text-indigo-600 transition-colors" title="Fit to Results">
                <Focus size={16} />
             </button>
         )}
      </div>
      ...
      {hoveredAsset && tooltipPos && (
        <div className="absolute z-[1000] pointer-events-none bg-white dark:bg-slate-900 px-3 py-2 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 text-xs transform -translate-x-1/2 -translate-y-full mt-[-10px] animate-fade-in" style={{ left: tooltipPos.x, top: tooltipPos.y - 15 }}>
           <p className="font-bold text-slate-800 dark:text-slate-200">{hoveredAsset.name}</p>
           <p className="text-slate-500 dark:text-slate-400">{hoveredAsset.category}</p>
        </div>
      )}
