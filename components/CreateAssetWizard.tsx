...
const CreateAssetWizard: React.FC<CreateAssetWizardProps> = ({ onClose }) => {
  ...
  const validateStep = (currentStep: number): boolean => {
    ...
    if (currentStep === 1) {
      ...
      if (!formData.owner) newErrors.owner = "Owner/Vendor is required";
    }
    ...
    if (currentStep === 2) {
      ...
      if (formData.category.includes('Rig')) {
         ...
         if (!formData.bollardPull) newErrors.bollardPull = "Bollard Pull/DWT is required";
      }
    }

    if (currentStep === 3) {
      if (!formData.lat || isNaN(parseFloat(formData.lat))) newErrors.lat = "Valid Latitude required";
      if (!formData.lng || isNaN(parseFloat(formData.lng))) newErrors.lng = "Valid Longitude required";
  ...
  const handleSubmit = () => {
    ...
    // DOMAIN LOGIC: Validate Asset Readiness (BKI & Specs)
    try {
  ...
  return (
    <div className="flex flex-col h-[500px]">
          ...
          {[1, 2, 3].map((s) => (
            <div key={s} className="relative z-10 flex flex-col items-center gap-2">
              ...
              <span className={`text-[10px] font-bold uppercase tracking-wider ${step >= s ? 'text-indigo-600 dark:text-white' : 'text-slate-400'}`}>
                {s === 1 ? 'General' : s === 2 ? 'Specs' : 'Location'}
              </span>
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-5 animate-fade-in">
            <div className="grid grid-cols-2 gap-4">
               <InputField label="Asset Name" field="name" value={formData.name} onChange={handleChange} error={errors.name} placeholder="e.g. Deepwater Horizon II" />
               <div className="space-y-1.5">
            ...
            <InputField label="Owner / Vendor (KKKS)" field="owner" value={formData.owner} onChange={handleChange} error={errors.owner} placeholder="e.g. Pertamina Hulu Energi" />
            <div className="p-4 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl flex flex-col items-center justify-center text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer group">
               ...
               <span className="text-xs font-bold">Upload Asset Image</span>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5 animate-fade-in">
             <div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-lg text-xs mb-2">
                <Info size={16} /><span>Configuring specs for <strong>{formData.category}</strong></span>
             </div>
             <div className="grid grid-cols-2 gap-4">
                <InputField label="Manufacturer" field="manufacturer" value={formData.manufacturer} onChange={handleChange} error={errors.manufacturer} placeholder="e.g. Keppel FELS" />
                <InputField label="Year Built" field="yearBuilt" type="number" value={formData.yearBuilt} onChange={handleChange} error={errors.yearBuilt} />
             </div>
        ...
        {step === 3 && (
          <div className="space-y-5 animate-fade-in">
             <div className="bg-slate-100 dark:bg-slate-800 h-32 rounded-xl flex items-center justify-center text-slate-400 border border-slate-200 dark:border-slate-700 mb-4 relative overflow-hidden">
                <MapPin size={32} /><span className="ml-2 text-xs font-bold">Pin location on map</span>
             </div>
             <InputField label="Location Name (Block/Area)" field="locationName" value={formData.locationName} onChange={handleChange} error={errors.locationName} placeholder="e.g. Natuna Sea Block A" />
             <div className="grid grid-cols-2 gap-4">
      ...
      <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-between bg-slate-50 dark:bg-slate-950">
         {step > 1 ? (
            <button onClick={handleBack} className="px-6 py-2.5 rounded-lg font-bold text-sm text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors flex items-center gap-2"><ChevronLeft size={16} /> Back</button>
         ) : <button onClick={onClose} className="px-6 py-2.5 rounded-lg font-bold text-sm text-slate-500 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors">Cancel</button>}
         {step < 3 ? (
            <button onClick={handleNext} className="px-6 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-lg font-bold text-sm shadow-sm hover:opacity-90 transition-opacity flex items-center gap-2">Next Step <ChevronRight size={16} /></button>
         ) : (
            <button onClick={handleSubmit} className="px-8 py-2.5 bg-emerald-600 text-white rounded-lg font-bold text-sm shadow-sm hover:bg-emerald-700 transition-opacity flex items-center gap-2"><Save size={16} /> Register Asset</button>
         )}
      </div>
    </div>
  );
