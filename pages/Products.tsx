...
const Products: React.FC = () => {
  ...
  }, [assets, activeTab, searchTerm, statusFilter, minYear, selectedManufacturer]);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto animate-fade-in pb-28">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Master Data Aset</h1>
          <p className="text-slate-500 text-sm">Centralized asset catalog and technical specifications.</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col lg:flex-row gap-4 justify-between bg-white dark:bg-slate-900">
           <div className="relative w-full lg:w-96">
              ...
              <input type="text" placeholder="Search assets..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:border-indigo-500 transition-all" />
           </div>
           <div className="flex gap-2">
              <button onClick={() => setIsFilterOpen(!isFilterOpen)} className={`flex items-center gap-2 px-3 py-2 border rounded-lg text-sm font-medium transition-colors ${isFilterOpen ? 'bg-slate-100 border-slate-300 text-slate-900' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                 <SlidersHorizontal size={14} /> Filters {isFilterOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
              </button>
           </div>
        </div>
        ...
        {isFilterOpen && (
           <div className="p-4 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-800 grid grid-cols-1 md:grid-cols-4 gap-4 animate-slide-in">
              <div>
                 ...
                 <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} className="w-full p-2 text-sm border rounded-lg bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 outline-none"><option value="All">All Status</option><option value="Active">Active</option><option value="Inactive">Inactive</option></select>
              </div>
              <div>
                 <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Manufacturer</label>
      ...
      {selectedForCompare.length > 0 && (
         <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white rounded-xl shadow-xl p-3 flex items-center gap-4 border border-slate-700">
            <span className="text-sm font-bold pl-2">{selectedForCompare.length} Selected</span>
            <div className="h-6 w-px bg-slate-700"></div>
            <button onClick={() => setSelectedForCompare([])} className="text-xs text-slate-400 hover:text-white">Cancel</button>
            <button onClick={() => selectedForCompare.length > 1 && navigate(`/compare?ids=${selectedForCompare.join(',')}`)} disabled={selectedForCompare.length < 2} className={`px-4 py-1.5 rounded-lg text-xs font-bold ${selectedForCompare.length > 1 ? 'bg-white text-slate-900' : 'bg-slate-800 text-slate-500'}`}>Compare</button>
         </div>
      )}
