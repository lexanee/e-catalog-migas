    ...
    <div className="p-6 max-w-7xl mx-auto animate-fade-in pb-20">
       <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
             ...
             <p className="text-slate-500 text-sm">CIVD database and performance monitoring.</p>
          </div>
          <div className="relative">
             <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4" />
             <input type="text" placeholder="Search vendor..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-sm outline-none focus:border-indigo-500 w-64" />
          </div>
       </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
             ...
             <div><p className="text-xs text-slate-500 uppercase font-bold">Verified</p><p className="text-xl font-bold text-slate-900 dark:text-white">{vendors.filter(v => v.status === 'Verified').length}</p></div>
          </div>
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
             <div className="p-2 bg-amber-50 text-amber-600 rounded-lg"><FileCheck size={20} /></div>
             <div><p className="text-xs text-slate-500 uppercase font-bold">Avg CSMS</p><p className="text-xl font-bold text-slate-900 dark:text-white">82.5</p></div>
          </div>
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
             <div className="p-2 bg-rose-50 text-rose-600 rounded-lg"><Ban size={20} /></div>
             <div><p className="text-xs text-slate-500 uppercase font-bold">Suspended</p><p className="text-xl font-bold text-slate-900 dark:text-white">{vendors.filter(v => v.status === 'Suspended').length}</p></div>
          </div>
       </div>
       ...
       <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          <table className="w-full text-left text-sm">
             <thead className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 text-slate-500 text-xs uppercase font-bold">
                <tr><th className="px-6 py-3">Vendor Name</th><th className="px-6 py-3">Category</th><th className="px-6 py-3">Status</th><th className="px-6 py-3">Rating</th><th className="px-6 py-3 text-right">Action</th></tr>
             </thead>
                   ...
                   <tr key={vendor.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-6 py-3 text-right">
                         <RBACWrapper allowedRoles={['admin']}>
                            <button onClick={() => { setSelectedVendor(vendor); analyzeVendorRisk(); }} className="text-indigo-600 font-bold text-xs hover:underline flex items-center justify-end gap-1"><Sparkles size={12} /> Audit</button>
                         </RBACWrapper>
                      </td>
                   </tr>
                ))}
       ...
       <Modal isOpen={isAiModalOpen} onClose={() => setIsAiModalOpen(false)} title="Vendor Audit">
          <div className="p-4">
             {isAnalyzing ? <div className="py-8 text-center"><Loader2 size={32} className="animate-spin mx-auto mb-2 text-slate-400" /><p className="text-sm text-slate-500">Auditing...</p></div> : <div className="text-sm whitespace-pre-line leading-relaxed">{riskReport}</div>}
          </div>
       </Modal>
    </div>
  );
