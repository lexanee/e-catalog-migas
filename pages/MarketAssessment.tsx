...
7. Plain text format for easy copying.`
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto animate-fade-in pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <div className="flex items-center gap-3">
             <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Market Assessment</h1>
          ...
          <p className="text-slate-500 text-sm mt-1">Pre-tender availability check and technical screening.</p>
        </div>
           ...
           {isSaved ? (
              ...
              <div className="flex gap-2">
                 <button onClick={generateBeritaAcara} disabled={!isCalculated} className="px-4 py-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 rounded-lg text-sm font-bold hover:bg-indigo-100 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                    <Sparkles size={16} /> AI Berita Acara
                 </button>
         ...
         <div className="lg:col-span-1 space-y-6">
            <div className={`bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-opacity ${isSaved ? 'opacity-75 pointer-events-none' : ''}`}>
               <h3 className="font-bold text-sm text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                  <Filter size={16} className="text-indigo-600" /> Assessment Criteria
               </h3>
               ...
               <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                     <label className="text-xs font-bold text-slate-500 uppercase">Asset Category</label>
                  ...
                  <div className="space-y-1.5">
                     <label className="text-xs font-bold text-slate-500 uppercase">Required Period</label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div className="space-y-1.5">
                        <label className="text-xs font-bold text-slate-500 uppercase">Min Year</label>
                  ...
                  <div className="space-y-1.5">
                     <label className="text-xs font-bold text-slate-500 uppercase">Region (Optional)</label>
            ...
            {isCalculated && (
                <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl">
                    <h4 className="font-bold text-emerald-800 text-sm mb-1">Analysis Complete</h4>
                    <p className="text-xs text-emerald-700">{assessment.candidates.length} assets found matching criteria and availability window.</p>
                </div>
            )}
         </div>
         ...
         <div className="lg:col-span-3">
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden min-h-[600px] flex flex-col">
               <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                  <h3 className="font-bold text-slate-800 dark:text-white text-sm uppercase tracking-wider">Candidate Assets</h3>
               ...
               {assessment.candidates.length === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-10">
                     ...
                     <p className="text-xs mt-1">Adjust filters and run analysis to see available assets.</p>
                  </div>
               ) : (
                     ...
                     <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 text-slate-500 text-xs uppercase font-bold sticky top-0">
                           <tr>
                              ...
                              <th className="px-6 py-3">Technical Specs</th>
                              <th className="px-6 py-3">Location</th>
                              <th className="px-6 py-3">Status</th>
                              <th className="px-6 py-3 text-right">Rate Estimate</th>
                           </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                           {assessment.candidates.map(asset => (
                              <tr key={asset.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                 ...
                                 <td className="px-6 py-4">
                                    <span className="px-2 py-1 rounded text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 uppercase">Available</span>
                                 </td>
      ...
      <Modal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} title="Lock Assessment">
         <div className="p-6">
            ...
            <div className="flex gap-3 justify-end">
               <button onClick={() => setIsConfirmOpen(false)} className="px-4 py-2 text-slate-500 font-bold text-sm hover:bg-slate-100 rounded-lg">Cancel</button>
      ...
      <Modal isOpen={isAiModalOpen} onClose={() => setIsAiModalOpen(false)} title="AI Report Generator">
         <div className="p-6 min-h-[400px]">
            {isGenerating ? (
               <div className="py-20 text-center">
                  <Loader2 size={40} className="animate-spin text-indigo-600 mx-auto mb-4" />
                  <h3 className="font-bold text-slate-800 dark:text-white">Generating Berita Acara...</h3>
                  <p className="text-sm text-slate-500 mt-2">Consulting Auditor Protocol & Analyzing Candidates</p>
               </div>
            ) : (
               <div className="space-y-4">
                  <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700">
                     ...
                     <button className="text-xs text-slate-500 hover:text-indigo-600 flex items-center gap-1 font-bold"><Copy size={12} /> Copy to Clipboard</button>
                  </div>
                  ...
                  <div className="flex justify-end">
                     <button onClick={() => setIsAiModalOpen(false)} className="px-6 py-2 bg-slate-900 text-white rounded-lg text-sm font-bold">Close</button>
                  </div>
               </div>
            )}
         </div>
