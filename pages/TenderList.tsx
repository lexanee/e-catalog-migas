...
const TenderList: React.FC = () => {
  ...
  const generateSOW = async () => {
     ...
     try {
        const prompt = `Write a professional Scope of Work (SOW) description for an oil & gas tender package containing: ${selectedAssets.join(', ')}. Include technical requirements and safety compliance. Max 100 words.`;
        ...
        if (process.env.API_KEY) {
           ...
           setTenderDesc(`Provision of ${selectedAssets.length} asset(s) including ${selectedAssets[0]}. Contractor shall provide fully crewed and operational units compliant with SKK Migas PTK-007 rev.5.`);
        }
        setTenderTitle(`Procurement of ${selectedAssets.length} Unit(s) - ${new Date().getFullYear()}`);
        ...
        setTenderDesc("Failed to generate description.");
     } finally {
  ...
  const evaluateBids = async () => {
     ...
     try {
        if (process.env.API_KEY) {
           const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
           const response = await ai.models.generateContent({ model: 'gemini-3-flash-preview', contents: "Evaluate bids for tender..." });
           setAiContent(response.text || "No evaluation.");
        } else {
           await new Promise(r => setTimeout(r, 1500));
           setAiContent(`**Evaluation Result:**\nRecommendation: ${selectedTender.bids?.[0]?.vendorName || 'None'} based on technical/commercial balance.`);
        }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto animate-fade-in pb-24">
       <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
             <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Procurement Tenders</h1>
             <p className="text-slate-500 text-sm mt-1">Manage tender packages, approvals, and sealed bidding.</p>
          </div>
       ...
       <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm min-h-[600px] flex flex-col">
          <div className="p-2 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-10 flex gap-1">
             <button onClick={() => setActiveTab('All')} className={`text-xs font-bold px-4 py-2 rounded-lg transition-all ${activeTab === 'All' ? 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}>All</button>
          ...
          <div className="flex-1 p-4 space-y-3 bg-slate-50 dark:bg-slate-950">
             {filteredTenders.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-slate-400">
                   ...
                   <p className="text-sm font-medium">No tenders found in this category.</p>
                </div>
             ) : (
                            ...
                            <div className="flex gap-4">
                               ...
                               <div>
                                  <div className="flex items-center gap-2 mb-1">
                                     ...
                                     {sealed && <span className="text-[10px] font-bold px-2 py-0.5 rounded border border-amber-200 bg-amber-50 text-amber-700 flex items-center gap-1"><Lock size={8} /> SEALED</span>}
                                  </div>
                                  <div className="flex items-center gap-4 text-xs text-slate-500">
                                     ...
                                     <span className="flex items-center gap-1"><Clock size={12} /> Opening: {tender.bidOpeningDate || 'TBD'}</span>
                                  </div>
                               </div>
                            </div>
                            <div className="text-right">
                               <p className="text-xs font-bold text-slate-400 uppercase mb-0.5">Nilai HPS</p>
                   ...
                   <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-xl flex items-center justify-between">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            ...
                            <p className="text-xs text-amber-700 dark:text-amber-300">Opening Date: {selectedTender.bidOpeningDate}</p>
                         </div>
                      </div>
                      {canOpenBids(selectedTender.bidOpeningDate) ? (
                          ...
                          <button disabled className="px-3 py-1.5 bg-slate-100 text-slate-400 font-bold rounded-lg text-xs flex items-center gap-2 cursor-not-allowed">
                             <Lock size={12} /> Locked until {selectedTender.bidOpeningDate}
                          </button>
                      )}
                   </div>
                )}
                ...
                <div>
                   <div className="flex justify-between items-center mb-4">
                      ...
                      {selectedTender.bids && selectedTender.bids.length > 0 && !isTenderSealed(selectedTender) && (
                         <button onClick={evaluateBids} className="text-xs font-bold text-indigo-600 hover:bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 flex items-center gap-1 transition-colors"><BrainCircuit size={14} /> AI Analysis</button>
                      )}
                   </div>
                   ...
                   {selectedTender.bids && selectedTender.bids.length > 0 ? (
                               ...
                               <div key={i} className="flex justify-between items-center p-4 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-900 hover:shadow-sm transition-shadow">
                                  <div>
                                     ...
                                     <div className="flex items-center gap-2 mt-1">
                                        {sealed ? (
                                           <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-slate-100 text-slate-500">Nilai Hidden</span>
                                        ) : (
                                           <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${bid.complianceScore && bid.complianceScore > 80 ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>Nilai: {bid.complianceScore}</span>
                                        )}
                                  ...
                                  <div className="flex items-center gap-4">
                                     ...
                                     {selectedTender.status === 'Published' && !sealed && (
                                        <button onClick={() => { awardTender(selectedTender.id, bid.vendorName, bid.bidAmount, requests); setSelectedTenderId(null); }} className="p-2 bg-slate-900 text-white rounded-lg hover:bg-slate-700 transition-colors tooltip" title="Award Contract">
                                           <Trophy size={16} />
                                        </button>
                                     )}
                                  </div>
                      ...
                      <div className="p-8 text-center border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 dark:bg-slate-900">
                         ...
                         <p className="text-sm font-medium text-slate-500">No bids submitted yet.</p>
                      </div>
                   )}
                </div>
             </div>
       ...
       <Modal isOpen={isCreateModalOpen} onClose={() => { setIsCreateModalOpen(false); setWizardStep(1); setSelectedRequestIds([]); }} title="Create Tender Package">
                ...
                {wizardStep === 1 ? (
                   <div className="space-y-4">
                      <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                         <p className="text-xs text-slate-500 mb-3 font-medium">Available Approved Enquiries</p>
                         {availableRequests.length === 0 ? (
                            <p className="text-center text-slate-400 text-sm italic py-4">No approved enquiries available for tendering.</p>
                         ) : (
                   ...
                   <div className="space-y-4">
                      <div className="space-y-2">
                         ...
                         <input value={tenderTitle} onChange={(e) => setTenderTitle(e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-lg text-sm font-bold outline-none focus:border-indigo-500" placeholder="e.g., Annual Rig Procurement" />
                      </div>

                      <div className="space-y-2">
                         <label className="text-xs font-bold text-slate-500 uppercase">Tanggal Pembukaan (Opening Date)</label>
                         ...
                         <p className="text-[10px] text-slate-400">Bids will remain encrypted until this date.</p>
                      </div>
                      
                      <div className="space-y-2 relative">
                         <label className="text-xs font-bold text-slate-500 uppercase flex justify-between">
                            Lingkup Kerja (SOW) 
                            {isGenerating && <span className="text-indigo-600 flex items-center gap-1"><Loader2 size={10} className="animate-spin" /> Generating...</span>}
                         </label>
                         <textarea value={tenderDesc} onChange={(e) => setTenderDesc(e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:border-indigo-500 min-h-[100px]" placeholder="Detailed description..." />
                         <button onClick={generateSOW} className="absolute right-2 top-8 p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-md" title="Regenerate with AI"><Wand2 size={14} /></button>
                      </div>
                      ...
                      <div className="flex justify-between pt-4">
                         <button onClick={() => setWizardStep(1)} className="px-4 py-2 text-slate-500 font-bold text-sm hover:text-slate-800">Back</button>
                         <button onClick={handleCreateTender} className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg font-bold text-sm hover:bg-indigo-700 shadow-sm flex items-center gap-2">
                            <FileText size={16} /> Publish Tender
                         </button>
                      </div>
                   </div>
                )}
