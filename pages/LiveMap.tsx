         ...
         <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 md:rounded-2xl shadow-2xl h-full md:h-auto md:max-h-[85vh] overflow-y-auto p-5 custom-scrollbar flex flex-col">
            <div className="flex items-center justify-between mb-6">
               <div className="flex items-center gap-3">
                  ...
                  <div><h1 className="font-bold text-slate-900 dark:text-white text-base tracking-tight">GeoCommand</h1><p className="text-xs text-slate-500 font-medium">Live Asset Tracking</p></div>
               </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
               <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
                  <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Fleet Size</span>
               ...
               <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-100 dark:border-emerald-800/50">
                  <span className="text-[10px] uppercase font-bold text-emerald-600 block mb-1">Active</span>
                  <span className="text-2xl font-bold text-emerald-700 dark:text-emerald-400">{activeCount}</span>
               </div>
            </div>

            <div className="space-y-1 mb-6">
               <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 px-1">Asset Types</p>
               {[
            ...
            <div className="space-y-3 border-t border-slate-100 dark:border-slate-800 pt-4">
               <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-1">Layer Control</p>
               {[{label: 'Working Areas', state: showWK, toggle: setShowWK}, {label: 'Safety Zones', state: showZones, toggle: setShowZones}, {label: 'Logistics Routes', state: showLogistics, toggle: setShowLogistics}].map((opt, i) => (
            ...
            <button onClick={generateRiskReport} className="w-full mt-6 py-3 bg-rose-600 text-white rounded-xl font-bold text-sm shadow-md shadow-rose-200 dark:shadow-none hover:bg-rose-700 transition-all flex justify-center items-center gap-2 active:scale-95">
               <ShieldAlert size={16} /> Run Threat Scan
            </button>
         </div>
      </div>
         ...
         <div className="absolute top-24 right-6 z-30 w-full md:w-80 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-slide-in">
            ...
            <div className="p-4 space-y-4">
               ...
               <div className="space-y-2 pt-2">
                  <button onClick={() => navigate(`/product/${selectedAsset.id}`)} className="w-full py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold flex justify-center items-center gap-2 hover:bg-indigo-700 transition-colors">View Dashboard <ChevronRight size={14} /></button>
                  <button onClick={() => navigate(`/request/${selectedAsset.id}`)} className="w-full py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">Request Quote</button>
               </div>
            </div>
         </div>
      )}

      <Modal isOpen={isRiskModalOpen} onClose={() => setIsRiskModalOpen(false)} title="AI Threat Analysis">
         <div className="p-6">
            {isAnalyzingRisk ? <div className="py-8 text-center"><Loader2 size={32} className="animate-spin mx-auto text-slate-400" /><p className="text-sm mt-2 text-slate-500 font-medium">Scanning satellite data...</p></div> : <div className="text-sm whitespace-pre-line leading-relaxed text-slate-700 dark:text-slate-300">{riskReport}</div>}
         </div>
      </Modal>
    </div>
  );
