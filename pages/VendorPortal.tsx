...
const VendorPortal: React.FC = () => {
  ...
  const generateProposal = async () => {
     ...
     try {
        ...
        Focus on safety record (CSMS) and technical compliance. Max 100 words.`;
        ...
        if (process.env.API_KEY) {
           ...
           setProposalText(`We are pleased to submit our proposal for ${selectedTender.name}. Our fleet meets all technical specifications outlined in the SOW, ensuring full compliance with PTK-007. With a CSMS score of 92 and zero LTI in the last 12 months, we guarantee operational excellence and safety.`);
        }
     } catch (e) { setProposalText("Error generating proposal."); } finally { setIsGenerating(false); }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto animate-fade-in pb-20">
       <div className="mb-8 flex justify-between items-end">
          <div>
             ...
             <p className="text-slate-500 text-sm mt-1">Managed by {user?.name}</p>
          </div>
          <div className="flex items-center gap-2 bg-indigo-50 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-indigo-100 dark:border-slate-700">
             ...
             <span className="text-xs font-bold text-indigo-700 dark:text-indigo-300">CIVD Verified</span>
          </div>
       </div>
       ...
       <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden min-h-[600px] flex flex-col">
          <div className="border-b border-slate-200 dark:border-slate-800 px-4 pt-2 flex gap-1 bg-slate-50 dark:bg-slate-950">
             {['Tenders', 'Enquiries', 'Contracts'].map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab as any)} className={`px-6 py-3 rounded-t-lg text-sm font-bold border-t border-x border-b-0 transition-all ${activeTab === tab ? 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-indigo-600 dark:text-white -mb-px' : 'border-transparent text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                   {tab}
                </button>
             ))}
          </div>
                ...
                <div className="space-y-4">
                   {publishedTenders.length === 0 ? (
                      <div className="p-12 text-center text-slate-400">
                         ...
                         <p>No open tender packages available.</p>
                      </div>
                   ) : (
                            ...
                            <div key={tender.id} className="border border-slate-200 dark:border-slate-700 p-5 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-sm transition-shadow">
                               <div>
                                  <div className="flex items-center gap-2 mb-1">
                                     <span className="bg-indigo-50 text-indigo-700 text-[10px] font-bold px-2 py-0.5 rounded border border-indigo-100 uppercase">Open Tender</span>
                                     <span className="text-slate-400 text-xs font-mono">{tender.id}</span>
                                     {isSealed && <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded border border-amber-100 bg-amber-50 text-amber-600"><Lock size={10} /> Sealed Bid</span>}
                                  </div>
                                  ...
                                  {isSealed && <p className="text-xs text-amber-600 mt-2 flex items-center gap-1"><Clock size={12} /> Opening Date: {tender.bidOpeningDate}</p>}
                               </div>
                               {hasBidded ? (
                                  ...
                                  <button onClick={() => setSelectedTender(tender)} className="px-6 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-lg text-sm font-bold shadow-sm hover:opacity-90 transition-opacity flex items-center gap-2">
                                     Prepare Bid <ArrowRight size={14} />
                                  </button>
                               )}
                            </div>
                         );
                ...
                <div className="space-y-4">
                   {vendorRequests.length === 0 ? (
                      <div className="p-12 text-center text-slate-400">
                         ...
                         <p>No asset enquiries pending.</p>
                      </div>
                   ) : (
                      ...
                      myContracts.map(contract => (
                         <div key={contract.id} className="bg-slate-50 dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700">
                            ...
                            <div className="space-y-2 text-sm text-slate-600 dark:text-slate-300 mb-4">
                               <div className="flex justify-between"><span>Value</span><span className="font-mono font-bold">IDR {contract.totalValue.toLocaleString()}</span></div>
                               <div className="flex justify-between"><span>Start Date</span><span>{contract.startDate}</span></div>
                            </div>
       ...
       <Modal isOpen={!!selectedTender} onClose={() => { setSelectedTender(null); setProposalStep(1); }} title="Smart Proposal Builder">
          <div className="p-6">
             {isSubmitted ? (
                <div className="py-10 text-center animate-fade-in">
                   ...
                   <h3 className="text-xl font-bold text-slate-900 dark:text-white">Proposal Submitted!</h3>
                   <p className="text-slate-500 mt-2">Your bid has been encrypted and sealed.</p>
                </div>
             ) : (
                   ...
                   {proposalStep === 1 ? (
                      <>
                         <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800 flex gap-3">
                            ...
                            <div>
                               <h4 className="font-bold text-indigo-900 dark:text-white text-sm">AI Tender Assistant</h4>
                               <p className="text-xs text-indigo-700 dark:text-indigo-300 mt-1">I can draft a compliant executive summary based on your CIVD profile and the SOW.</p>
                            </div>
                         </div>
                         <div className="space-y-3">
                            <label className="text-xs font-bold text-slate-500 uppercase">Commercial Offer (IDR)</label>
                      ...
                      <>
                         ...
                         <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                            <div className="flex justify-between items-center mb-2">
                               <span className="text-xs font-bold text-slate-500 uppercase">Uploaded Documents</span>
                               <button className="text-[10px] font-bold text-indigo-600 flex items-center gap-1"><Upload size={10} /> Add File</button>
                            </div>
                         ...
                         <div className="flex gap-3 pt-2">
                            <button onClick={() => setProposalStep(1)} className="px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg font-bold text-sm text-slate-500">Back</button>
                            <button onClick={handleBidSubmit} className="flex-1 py-2.5 bg-indigo-600 text-white rounded-lg font-bold text-sm hover:bg-indigo-700 shadow-sm">Submit Proposal</button>
                         </div>
                      </>
                   )}
                </div>
       ...
       <Modal isOpen={!!selectedRequest} onClose={() => setSelectedRequest(null)} title={`Quote: ${selectedRequest?.assetName}`}>
          <form onSubmit={handleEnquirySubmit} className="p-6 space-y-4">
             <div>
                <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Availability Confirmation</label>
                <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-lg text-emerald-700 text-sm font-medium flex items-center gap-2"><CheckCircle size={16} /> Asset Available for {selectedRequest?.dateFrom} - {selectedRequest?.dateTo}</div>
             </div>
             <div>
                <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Day Rate (IDR)</label>
             ...
             <button type="submit" className="w-full py-3 bg-indigo-600 text-white rounded-lg font-bold text-sm hover:bg-indigo-700">Send Quotation</button>
          </form>
       </Modal>
    </div>
  );
