  ...
  return (
    <div className="p-6 max-w-7xl mx-auto animate-fade-in pb-24">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
         <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Market Enquiries</h1>
            <p className="text-slate-500 text-sm mt-1">Manage asset availability surveys and vetting.</p>
         </div>
            ...
            <table className="w-full text-left text-sm">
               <thead className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 text-slate-500 text-xs uppercase font-bold sticky top-0">
                  <tr>
                     ...
                     <th className="px-6 py-3">Data Aset</th>
               ...
               <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredRequests.length === 0 ? (
                     <tr><td colSpan={6} className="p-10 text-center text-slate-400">No enquiries found matching your filters.</td></tr>
                     ...
                     <tr key={req.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer transition-colors group" onClick={() => setSelectedRequest(req)}>
                        ...
                        <td className="px-6 py-4 text-right"><span className="text-indigo-600 font-bold text-xs hover:underline opacity-0 group-hover:opacity-100 transition-opacity">View Details</span></td>
                     </tr>
                  ))}
               </tbody>
            </table>
        ...
        {selectedRequest && (
          <div className="space-y-6">
             <div className="bg-slate-50 dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 flex justify-between items-start">
                <div>
                   <span className="text-[10px] font-bold uppercase text-slate-400 mb-1 block">Aset Terkait (Subject Asset)</span>
             ...
             <div className="grid grid-cols-2 gap-6">
                <div>
                   <h4 className="text-xs font-bold text-slate-500 uppercase mb-3">KKKS Information</h4>
             ...
             <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden flex flex-col h-64">
                ...
                <div className="p-4 flex-1 overflow-y-auto bg-white dark:bg-slate-900 space-y-3">
                   ...
                   )) : <p className="text-xs text-slate-400 text-center italic mt-10">No messages yet.</p>}
                </div>
                <div className="p-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex gap-2">
                   <input className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs outline-none focus:border-indigo-500 transition-colors" placeholder="Type internal note..." />
                   <button className="p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700"><Send size={14} /></button>
                </div>
             </div>

             <div className="flex gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                {/* Security Restriction: Only Admins can shortlist/award */}
                {selectedRequest.status === 'Review' && (
                  <RBACWrapper allowedRoles={['admin']}>
                     <button onClick={() => handleStatusChange('Approved')} disabled={selectedRequest.techStatus !== 'Valid'} className="flex-1 py-2.5 bg-slate-900 text-white rounded-lg font-bold text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-800 transition-colors shadow-sm">Shortlist Candidate</button>
                  </RBACWrapper>
                )}
                
                {selectedRequest.status === 'Review' && (
                   <RBACWrapper allowedRoles={['technical', 'admin']}>
                      <button onClick={() => setIsTechModalOpen(true)} className="flex-1 py-2.5 bg-indigo-600 text-white rounded-lg font-bold text-sm hover:bg-indigo-700 transition-colors shadow-sm">Technical Validation</button>
                   </RBACWrapper>
                )}
                <button onClick={() => handleStatusChange('Rejected')} className="px-6 py-2.5 border border-slate-200 bg-white text-slate-600 rounded-lg font-bold text-sm hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 transition-colors">Reject</button>
             </div>
          </div>
        )}
      </Modal>
      ...
      <Modal isOpen={isTechModalOpen} onClose={() => setIsTechModalOpen(false)} title="Validasi Teknis">
         <div className="space-y-4 p-2">
            <p className="text-sm text-slate-500">Provide assessment notes for the engineering team.</p>
            <textarea className="w-full border border-slate-300 dark:border-slate-700 rounded-xl p-4 text-sm bg-white dark:bg-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none resize-none" rows={4} placeholder="Enter technical assessment details..." value={techNotes} onChange={(e) => setTechNotes(e.target.value)}></textarea>
            <div className="flex gap-3">
               <button onClick={() => { updateRequest(selectedRequest!.id, { techStatus: 'Invalid', techNotes }); setIsTechModalOpen(false); }} className="flex-1 py-2.5 border border-rose-200 bg-rose-50 text-rose-700 rounded-lg font-bold text-sm hover:bg-rose-100 transition-colors">Flag Issues</button>
               <button onClick={() => { updateRequest(selectedRequest!.id, { techStatus: 'Valid', techNotes }); setIsTechModalOpen(false); }} className="flex-1 py-2.5 bg-emerald-600 text-white rounded-lg font-bold text-sm hover:bg-emerald-700 transition-colors shadow-sm">Approve Asset</button>
            </div>
         </div>
      </Modal>
    </div>
