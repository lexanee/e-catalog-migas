...
import { ArrowLeft, CheckCircle, FileText, Building2, CalendarClock, Wrench, ShieldCheck, Activity, Power, Cpu, Gauge, Users, AlertTriangle, Sparkles, BrainCircuit, BarChart2, Package, ClipboardList, AlertCircle, ShoppingCart, Minus, Plus, MapPin, Factory, Calendar, File, Download, Eye, Clock, Upload, TrendingDown, X, Leaf, CloudRain, Zap, Loader2, ShieldAlert, HeartPulse, History, Edit, Flag, Save } from 'lucide-react';
...
const ProductDetail: React.FC = () => {
  ...
  const emissionHistory = useMemo(() => {
    return Array.from({ length: 7 }, (_, i) => ({
      day: `Day ${i + 1}`,
      co2: (asset?.co2Emissions || 0) + (Math.random() - 0.5) * 5
    }));
  }, [asset]);
  ...
  const runHSEAnalysis = async () => {
     ...
     try {
        const prompt = `
           ...
           Identify the primary operational risk and suggest one mitigation step. Be concise.
        `;
        ...
        if (process.env.API_KEY) {
           ...
           const level = asset.csmsScore > 80 ? "Low" : "Moderate";
           setHseReport(`**HSE Integrity Report:**\n\n**Risk Level: ${level}.** MTBF trend is stable. ${asset.incidentCount > 0 ? "Past incidents were minor mechanical failures." : "No recorded LTIs."}\n\n**Action:** Schedule preemptive inspection of secondary blowout preventers before spud date.`);
        }
  ...
  const handleUsePart = (partId: string) => {
    ...
    if (part && part.quantity <= part.minLevel) addNotification(asset.id, 'Low Stock Warning', `Inventory for ${part.name} is below minimum level.`, 'warning');
  };
  ...
  const handleCommand = (type: 'reboot' | 'scan' | 'fault') => {
    ...
    if (type === 'reboot') {
      setIsRebooting(true);
      setTimeout(() => { setIsRebooting(false); addNotification(asset.id, 'System Success', `${asset.name} rebooted.`, 'info'); }, 3000);
    }
    if (type === 'scan') {
      setIsScanning(true);
      setTimeout(() => { setIsScanning(false); addNotification(asset.id, 'Diagnostics Complete', `0 faults found.`, 'info'); }, 2000);
    }
    if (type === 'fault') {
      updateAsset(asset.id, { health: 30 });
      addNotification(asset.id, 'TEST', 'Simulated Health Drop to 30%', 'critical');
    }
  };
  ...
  const handleComplianceCheck = () => {
    ...
    try {
      ...
      addNotification(asset.id, 'Compliance Verified', 'Asset meets all technical and administrative requirements for active duty.', 'info');
    } catch (e: any) {
      setComplianceError(e.message);
      addNotification(asset.id, 'Compliance Failed', e.message, 'warning');
    }
  };
  ...
  const runAiAnalysis = async () => {
    ...
    try {
       const prompt = `Health status for ${asset.name}. Score: ${asset.health}%. Predictive analysis. Brief.`;
       if (process.env.API_KEY) {
          const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
          const response = await ai.models.generateContent({ model: 'gemini-3-flash-preview', contents: prompt });
          ...
          await new Promise(r => setTimeout(r, 1500));
          setAiResultText(`Health Score: ${asset.health}%. All systems within normal limits. Engine operating temp optimal. Recommended visual inspection of deck hydraulics.`);
       }
  ...
  const handleEditSubmit = () => {
     ...
     try {
        ...
        addNotification(asset.id, "Asset Updated", "Modifications saved and validated successfully.", "info");
     } catch (e: any) {
  ...
  const lowStockCount = asset.inventory?.filter(i => i.quantity <= i.minLevel).length || 0;
    ...
    <div className="animate-fade-in pb-12">
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-20">
         <div className="max-w-7xl mx-auto px-6 py-6">
            <Link to="/products" className="inline-flex items-center text-sm text-slate-500 hover:text-slate-800 dark:hover:text-white mb-4 transition-colors">
               <ArrowLeft size={16} className="mr-1" /> Back to Assets
            </Link>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
         ...
         <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
               ...
               <div><p className="text-xs text-slate-500 font-bold uppercase">Health</p><p className={`text-xl font-bold ${asset.health < 40 ? 'text-rose-600' : 'text-slate-900 dark:text-white'}`}>{asset.health}%</p></div>
            </div>
            <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
         ...
         {asset.maintenanceLog && asset.maintenanceLog.length > 0 && asset.maintenanceLog[0].title.includes('AUTO-WO') && (
            <div className="mb-6 p-4 bg-rose-50 border border-rose-100 text-rose-800 rounded-xl flex items-center justify-between shadow-sm animate-pulse-slow">
               <div className="flex items-center gap-3">
                  ...
                  <div>
                     <h3 className="font-bold">Active Maintenance Alert</h3>
                     <p className="text-xs">{asset.maintenanceLog[0].title} - {asset.maintenanceLog[0].description}</p>
                  </div>
               </div>
               <button className="px-4 py-2 bg-white text-rose-600 text-xs font-bold rounded-lg border border-rose-200 shadow-sm">View Work Order</button>
            </div>
         )}
         ...
         {complianceError && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-100 text-amber-800 rounded-xl flex items-center gap-3 shadow-sm animate-fade-in">
               ...
               <div>
                  <h3 className="font-bold">Compliance Validation Failed</h3>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               ...
               {activeTab === 'telemetry' && (
                 <div className="space-y-6 animate-fade-in">
                   ...
                   <div className="h-64 bg-slate-100 dark:bg-slate-900 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 relative">
                      <div className="absolute top-4 left-4 z-10 bg-white dark:bg-slate-900 px-3 py-1 rounded-lg text-xs font-bold shadow-sm border border-slate-100 dark:border-slate-700 text-slate-700 dark:text-slate-300">Live Position</div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                         <div className="flex justify-between mb-4">
                            <h4 className="text-sm font-bold text-slate-800 dark:text-white">Engine Temperature</h4>
                            <span className="text-xs text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded font-bold">Stable</span>
                         </div>
                         <div className="h-48">
                      ...
                      <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                         <div className="flex justify-between mb-4">
                            <h4 className="text-sm font-bold text-slate-800 dark:text-white">Hydraulic Pressure</h4>
                  ...
                  <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 animate-fade-in shadow-sm">
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 border border-slate-100 dark:border-slate-800 rounded-lg">
                           ...
                           <p className="text-3xl font-bold text-emerald-600">{asset.daysSinceIncident} <span className="text-sm font-medium text-slate-400">Days</span></p>
                        </div>
                        <div className="p-4 border border-slate-100 dark:border-slate-800 rounded-lg">
                        ...
                        {asset.inventory?.map(part => (
                           <div key={part.id} className="flex items-center justify-between p-3 border border-slate-100 dark:border-slate-800 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50">
                              ...
                              <div className="text-right">
                                 ...
                                 <button onClick={() => handleUsePart(part.id)} className="text-[10px] text-indigo-600 font-bold hover:underline">Use Item</button>
                              </div>
                           </div>
                        ))}
                     </div>
            ...
            <div className="lg:col-span-1 space-y-6">
               ...
               <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
                   <h3 className="font-bold text-sm text-slate-800 dark:text-white mb-4 flex items-center gap-2"><Flag size={14} /> Cabotage Status</h3>
                   <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg mb-4 text-center border border-slate-100 dark:border-slate-700">
                       <p className="text-xs text-slate-500 uppercase font-bold mb-1">Prioritas Cabotage</p>
                   ...
                   <button onClick={handleComplianceCheck} className="w-full py-2.5 border border-indigo-200 bg-indigo-50 text-indigo-700 rounded-lg text-xs font-bold hover:bg-indigo-100 transition-colors flex items-center justify-center gap-2">
                      <CheckCircle size={14} /> Validate Readiness
                   </button>
               </div>
               ...
               <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
                  ...
                  <div className="space-y-3">
                     <div className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-colors">
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Reboot System</span>
                     ...
                     <div className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-colors">
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Run Diagnostics</span>
                     ...
                     <button onClick={() => handleCommand('fault')} className="w-full flex justify-between items-center p-3 bg-rose-50 dark:bg-rose-900/20 rounded-lg border border-rose-100 dark:border-rose-800/50 hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-colors">
                        <span className="text-sm font-bold text-rose-700 dark:text-rose-400">Simulate Critical Fault</span>
      ...
      <Modal isOpen={isAiModalOpen} onClose={() => setIsAiModalOpen(false)} title="Predictive Analysis">
        <div className="p-6 text-center">
           {aiAnalysisStage === 'analyzing' ? (
              <div className="py-8"><div className="w-16 h-16 mx-auto border-4 border-slate-100 border-t-indigo-600 rounded-full animate-spin flex items-center justify-center text-xs font-bold text-indigo-600">{scanProgress}%</div><h3 className="text-lg font-bold mt-6 text-slate-800">Analyzing Telemetry...</h3></div>
           ) : (
              <div className="text-left">
                 <div className="bg-slate-50 p-5 rounded-xl flex items-start gap-4 mb-4 border border-slate-100">
                    ...
                    <div><h3 className="font-bold text-slate-800">Diagnostic Result</h3><p className="text-sm text-slate-600 mt-2 leading-relaxed">{aiResultText}</p></div>
                 </div>
              </div>
           )}
        </div>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Asset Details">
         <div className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
               <div>
                  <label className="text-xs font-bold text-slate-500 uppercase">Health (%)</label>
                  <input 
                     type="number"
            ...
            <div className="flex justify-end gap-2 pt-4">
               <button onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 border rounded-lg text-sm">Cancel</button>
               <button onClick={handleEditSubmit} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold flex items-center gap-2"><Save size={14} /> Save Changes</button>
            </div>
         </div>
      </Modal>
    </div>
