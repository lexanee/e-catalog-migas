...
interface AssetContextType {
  ...
  injectScenario: (type: 'failure_rig_a' | 'weather_natuna' | 'cyber_attack') => void;
  resetAssets: () => void;
}
...
export const AssetProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addNotification = (assetId: string, title: string, message: string, type: 'critical' | 'warning' | 'info') => {
  ...
  const injectScenario = (type: 'failure_rig_a' | 'weather_natuna' | 'cyber_attack') => {
    if (type === 'failure_rig_a') {
       setAssets(prev => prev.map(a => {
          if (a.id === '1') {
             return { ...a, health: 35, status: 'Inactive', incidentCount: a.incidentCount + 1, csmsScore: 40 };
          }
          return a;
       }));
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setAssets(currentAssets => currentAssets.map(asset => {
        ...
        const newTotalEmissions = asset.totalEmissions + (asset.co2Emissions / 86400 * 2) + emissionJitter;
  ...
  return (
    <AssetContext.Provider value={{ 
      assets, zones, notifications, savedAssessments, unreadCount, isSimulationPaused,
      markAsRead, markAllAsRead, addNotification, addAsset, updateAsset, deleteAsset, 
      saveFinalAssessment, toggleSimulation, injectScenario, resetAssets 
    }}>
      {children}
