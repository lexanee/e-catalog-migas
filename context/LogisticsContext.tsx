...
import { shorebases as initialShorebases } from '../mockData';
...
interface LogisticsContextType {
  ...
  updateShorebaseStock: (shorebaseId: string, item: string, qtyDelta: number) => void;
...
export const LogisticsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [shorebases, setShorebases] = useState<Shorebase[]>(initialShorebases);
  ...
  const createTransfer = (transfer: Transfer) => {
    ...
    setShorebases(prev => prev.map(sb => {
  ...
  const updateTransferStatus = (id: string, status: 'RECEIVED' | 'SHIPPING') => {
    setTransfers(prev => prev.map(t => t.id === id ? { ...t, status } : t));
  };

  const updateShorebaseStock = (shorebaseId: string, item: string, qtyDelta: number) => {
    setShorebases(prev => prev.map(sb => {
  ...
  const resetLogistics = () => {
    setShorebases(initialShorebases);
    setTransfers([]);
  };

  return (
    <LogisticsContext.Provider value={{ 
      shorebases, transfers, 
      createTransfer, updateTransferStatus, updateShorebaseStock,
      resetLogistics 
    }}>
      {children}
