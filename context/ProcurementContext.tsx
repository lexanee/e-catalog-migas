...
import { QuotationRequest, Tender, Contract, Vendor, TenderBid, AssessmentDoc } from '../types';
import { requestsData as initialRequests, vendors as initialVendors } from '../mockData';
...
interface ProcurementContextType {
  ...
  awardTender: (tenderId: string, vendorName: string, amount: number, associatedRequests: QuotationRequest[]) => void;
  
  // Contract Actions
  updateContract: (id: string, updates: Partial<Contract>) => void;
  
  // Assessment Actions
  saveAssessment: (doc: AssessmentDoc, status: 'DRAFT' | 'FINAL') => AssessmentDoc;
...
export const ProcurementProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const awardTender = (tenderId: string, vendorName: string, amount: number, associatedRequests: QuotationRequest[]) => {
    const tender = tenders.find(t => t.id === tenderId);
    if (!tender) return;

    updateTender(tenderId, { status: 'Closed' });

    const contractId = `CTR-${new Date().getFullYear()}-${Math.floor(Math.random()*10000)}`;
    const assetNames = associatedRequests.filter(r => tender.items.includes(r.id)).map(r => r.assetName);
    ...
    const newContract: Contract = {
      ...
      milestones: [
        { id: 'm1', label: 'Contract Signing', targetDate: new Date().toISOString().split('T')[0], status: 'Completed' },
        { id: 'm2', label: 'Mobilization', targetDate: '2024-05-15', status: 'In Progress' },
        { id: 'm3', label: 'Spud / Commencement', targetDate: '2024-06-01', status: 'Pending' },
        { id: 'm4', label: 'Project Completion', targetDate: '2025-05-01', status: 'Pending' }
      ]
    };
  ...
  return (
    <ProcurementContext.Provider value={{ 
      requests, tenders, contracts, vendors, assessments,
      addRequest, updateRequest, addTender, updateTender, 
      submitBid, awardTender, updateContract, saveAssessment, resetProcurement 
    }}>
      {children}
