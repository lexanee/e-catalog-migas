...
const AssetDetail: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'specs' | 'ref' | 'history' | 'telemetry' | 'inventory' | 'sustainability' | 'safety'>('specs');
  ...
  const handleAuditAction = (action: 'approve' | 'reject') => {
     ...
     if(action === 'approve') {
        ...
        if (!allChecked) {
            alert("Harap centang semua dokumen verifikasi terlebih dahulu.");
            return;
        }
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="space-y-6 animate-fade-in">
                     ...
                     <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                           <h3 className="font-bold text-lg text-slate-900 dark:text-white">Data Validasi / Validation Data</h3>
                           <FileCheck size={18} className="text-emerald-500" />
                        </div>
                        <div className="space-y-4">
                           <div className="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                              <span className="text-sm font-medium text-slate-500">Nomor IMO / IMO Number</span>
                              <span className="text-sm font-bold text-slate-900 dark:text-white font-mono">{asset.imoNumber || 'N/A'}</span>
                           </div>
                           <div className="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                              <span className="text-sm font-medium text-slate-500">Informasi Vendor / Vendor Information</span>
                              <span className="text-sm font-bold text-slate-900 dark:text-white">{vendor?.name || 'Unknown Vendor'}</span>
                           </div>
                           <div className="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                              <span className="text-sm font-medium text-slate-500">Status Vendor / Vendor Status</span>
                              <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${vendor?.status === 'Verified' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                                 {vendor?.status || 'Pending'}
                              </span>
                           </div>
                           <div className="flex justify-between items-start pt-2">
                              <span className="text-sm font-medium text-slate-500">Bukti Kepemilikan / Proof of Ownership</span>
                              <div className="text-right">
                                 <span className="block text-xs font-bold text-indigo-600 cursor-pointer hover:underline mb-1">Bukti Kepemilikan.pdf</span>
                     ...
                     <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 gap-y-6 gap-x-8 text-sm mb-6">
                           ...
                           <div>
                              <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Kategori / Tipe</label>
                              <p className="font-medium text-slate-900 dark:text-white">{asset.category} {asset.subType ? `(${asset.subType})` : ''}</p>
                           </div>
                           <div>
                              <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Pabrikan / Tahun</label>
                              <p className="font-medium text-slate-900 dark:text-white">{asset.manufacturer} / {asset.yearBuilt}</p>
                           </div>
                           <div>
                              <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Bendera (Flag)</label>
                              <p className="font-medium text-slate-900 dark:text-white">{asset.flagCountry || 'Indonesia'}</p>
                           </div>
                           <div>
                     ...
                     <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                           <h3 className="font-bold text-lg text-slate-900 dark:text-white">Informasi Pemilik (Owner)</h3>
                           <span className={`text-[10px] px-2 py-1 rounded border uppercase font-bold ${asset.ownerType === 'National' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>{asset.ownerType}</span>
                        </div>
                        {vendor ? (
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div className="p-3 border border-slate-100 dark:border-slate-800 rounded-lg flex items-start gap-3">
                                 ...
                                 <div>
                                    <p className="text-xs font-bold text-slate-500 uppercase">Nama Perusahaan</p>
                              ...
                              <div className="p-3 border border-slate-100 dark:border-slate-800 rounded-lg flex items-start gap-3">
                                 ...
                                 <div>
                                    ...
                                    <p className="font-medium text-slate-900 dark:text-white">{vendor.contactEmail || 'contact@vendor.com'}</p>
                                 </div>
                              </div>
                           </div>
                        ) : (
                           <p className="text-slate-400 italic">Data vendor tidak tersedia.</p>
                        )}
                     </div>
                  </div>
               )}
            ...
            <div className="lg:col-span-1 space-y-6">
               {showVerificationPanel && (
                  <div className="bg-white dark:bg-slate-900 rounded-xl border border-indigo-200 dark:border-indigo-900 shadow-lg shadow-indigo-100 dark:shadow-none p-5 relative overflow-hidden">
                     ...
                     <h3 className="font-bold text-sm text-indigo-900 dark:text-white mb-4 flex items-center gap-2">
                        <ClipboardCheck size={16} className="text-indigo-600" /> Verifikasi Teknis
                     </h3>
                     <div className="space-y-3 mb-6">
                        <label className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer transition-colors">
                           <input type="checkbox" checked={verificationChecks.doc_skpp} onChange={() => toggleCheck('doc_skpp')} className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500" />
                           <span className="text-xs font-medium text-slate-700 dark:text-slate-300">Dokumen SKPP Migas Valid</span>
                        </label>
                        <label className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer transition-colors">
                           <input type="checkbox" checked={verificationChecks.doc_bki} onChange={() => toggleCheck('doc_bki')} className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500" />
                           <span className="text-xs font-medium text-slate-700 dark:text-slate-300">Sertifikat Klasifikasi (BKI)</span>
                        </label>
                     </div>
                     <div className="flex gap-2">
                        <button onClick={() => handleAuditAction('reject')} className="flex-1 py-2.5 border border-rose-200 text-rose-600 rounded-lg text-xs font-bold hover:bg-rose-50 transition-colors">Tolak (Revisi)</button>
                        <button onClick={() => handleAuditAction('approve')} disabled={!Object.values(verificationChecks).every(Boolean)} className="flex-1 py-2.5 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                           Approve Asset
                        </button>
                     </div>
                  </div>
               ...
               <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
                  <h3 className="font-bold text-sm text-slate-800 dark:text-white mb-4">Ringkasan Spesifikasi</h3>
                  <div className="space-y-3 text-sm">
                     ...
                     <div className="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                        ...
                        <span className="font-medium text-slate-800 dark:text-slate-200">{asset.capacityString}</span>
                     </div>
                     <div className="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
