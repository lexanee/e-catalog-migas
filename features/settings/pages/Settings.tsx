...
import { Activity, Moon, Globe, Database, Server, Building2, FileCheck, Save } from 'lucide-react';
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto animate-fade-in pb-20">
      ...
      <div className="flex gap-1 mb-6 border-b border-slate-200 dark:border-slate-800">
         <button onClick={() => setActiveTab('General')} className={`px-4 py-2 text-sm font-bold border-b-2 transition-colors ${activeTab === 'General' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500'}`}>Umum</button>
         {user?.role === 'vendor' && (
            <button onClick={() => setActiveTab('Company')} className={`px-4 py-2 text-sm font-bold border-b-2 transition-colors ${activeTab === 'Company' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500'}`}>Profil Perusahaan (CIVD)</button>
         )}
      </div>
      ...
      {activeTab === 'General' ? (
         ...
         <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm max-w-2xl animate-fade-in">
            <h3 className="font-bold text-lg mb-6 flex items-center gap-2 text-slate-800 dark:text-white"><Building2 className="text-indigo-600" /> Data Perusahaan</h3>
            <div className="space-y-5">
               <div>
                  <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5">Nama Perusahaan</label>
                  <input type="text" value={vendorData.companyName} onChange={(e) => setVendorData({...vendorData, companyName: e.target.value})} className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded-lg text-sm bg-slate-50 dark:bg-slate-800 outline-none focus:border-indigo-500" />
               </div>
               <div className="grid grid-cols-2 gap-6">
                  <div>
                     <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5">Nomor CIVD</label>
                     <div className="relative">
                        <FileCheck size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                        <input type="text" value={vendorData.civd} onChange={(e) => setVendorData({...vendorData, civd: e.target.value})} className="w-full pl-9 pr-3 py-3 border border-slate-200 dark:border-slate-700 rounded-lg text-sm bg-slate-50 dark:bg-slate-800 outline-none focus:border-indigo-500 font-mono" />
                     </div>
                  </div>
                  <div>
                     <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5">NPWP</label>
                     <input type="text" value={vendorData.npwp} onChange={(e) => setVendorData({...vendorData, npwp: e.target.value})} className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded-lg text-sm bg-slate-50 dark:bg-slate-800 outline-none focus:border-indigo-500 font-mono" />
                  </div>
               </div>
               <div>
                  <label className="text-xs font-bold text-slate-500 uppercase block mb-1.5">Skor CSMS Terakhir</label>
                  <div className="flex items-center gap-4">
                     <input type="number" value={vendorData.csms} onChange={(e) => setVendorData({...vendorData, csms: e.target.value})} className="w-24 p-3 border border-slate-200 dark:border-slate-700 rounded-lg text-sm bg-slate-50 dark:bg-slate-800 outline-none focus:border-indigo-500 font-bold text-center" />
                     <span className={`text-xs font-bold px-3 py-1.5 rounded border ${parseInt(vendorData.csms) > 80 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                        {parseInt(vendorData.csms) > 80 ? 'Resiko Rendah' : 'Resiko Sedang'}
                     </span>
                  </div>
               </div>
               <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-end">
                  <button onClick={handleVendorSave} className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg font-bold text-sm hover:bg-indigo-700 flex items-center gap-2">
