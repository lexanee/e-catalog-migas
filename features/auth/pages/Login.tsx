...
const Login: React.FC = () => {
  ...
  const [email, setEmail] = useState('scm@skkmigas.go.id');
  ...
  const handleRoleSwitch = (role: UserRole) => {
    setActiveRole(role);
    if (role === 'scm') setEmail('scm@skkmigas.go.id');
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            ...
            {[
              ...
              { id: 'vendor', label: 'Penyedia', icon: Briefcase }
               ...
               <button 
                  ...
                  className={`py-4 text-[10px] font-bold uppercase tracking-wider transition-all flex flex-col items-center gap-1.5 ${activeRole === role.id ? 'bg-slate-50 dark:bg-slate-800 text-indigo-700 dark:text-white border-b-2 border-indigo-600' : 'bg-white dark:bg-slate-900 text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-950 border-b-2 border-transparent'}`}
               >
                  <role.icon size={18} />
                  {role.label}
               </button>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide">Alamat Email</label>
              <div className="relative group">
                ...
                <input 
                  ...
                  placeholder="user@skkmigas.go.id"
               />
              </div>
            </div>
