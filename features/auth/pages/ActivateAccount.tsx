...
import { Hexagon, Lock, CheckCircle, AlertCircle, ArrowRight } from 'lucide-react';
...
const ActivateAccount: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!validatePassword(password)) {
      setError('Kata sandi tidak valid. Harap ikuti kriteria keamanan (Min 8 char, 1 Besar, 1 Kecil, 1 Angka).');
      return;
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6 animate-fade-in">
        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800 p-8 text-center">
           ...
           <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Aktivasi Berhasil</h2>
           <p className="text-slate-500 text-sm mb-6">
              Selamat, akun Anda sudah aktif.<br/>
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 transition-colors p-6">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
           ...
           <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Aktivasi Akun</h1>
           <p className="text-slate-500 text-sm mt-1">Buat kata sandi untuk mengamankan akun Anda.</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide">Kata Sandi Baru</label>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide">Ulangi Kata Sandi</label>
              <div className="relative group">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors w-4 h-4" />
                <input 
                  type="password" 
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className={`w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-lg transition-all flex items-center justify-center gap-2 mt-4 shadow-sm ${isLoading ? 'opacity-70 cursor-not-allowed' : 'hover:scale-[1.01]'}`}
            >
